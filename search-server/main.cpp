...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate) const {
        ...
        matched_documents.swap(matched_documents);
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        if (text.empty()) {
            throw invalid_argument("Empty word found in query");
        }
        bool is_minus = false;
        if (text[0] == '-') {
            is_minus = true;
        ...
        if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
            throw invalid_argument("Minus word is empty or include '-' sign or 0-31 symbols"s);
        }
